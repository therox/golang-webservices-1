...
func TestMyApi(t *testing.T) {
	...
	cases := []Case{
		...
		Case{ // сработала валидация - логин не должен быть пустым (2)
			Path:   ApiUserProfile,
			Query:  "",
		...
		Case{ // получили ошибку общего назначения - ваш код сам подставил 500 (3)
			Path:   ApiUserProfile,
		...
		Case{ // получили специализированную ошибку - ваш код поставил статус 404 оттуда (4)
			Path:   ApiUserProfile,
		...
		// ------
		Case{ // это должен ответить ваш ServeHTTP - если ему пришло что-то неизвестное (например когда он обрабатывает /user/) (5)
			Path:   "/user/unknown",
			Query:  "login=not_exist_user",
			Status: http.StatusNotFound,
			Result: CR{
		...
		Case{ // юзер действительно создался (7)
			Path:   ApiUserProfile,
		...
		Case{ // обрабатываем неизвестную ошибку  (18)
			Path:   ApiUserCreate,
			Method: http.MethodPost,
